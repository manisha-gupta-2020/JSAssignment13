...
const createSlideshow = function () {
    ...
    const displayNextImage = function () {
        ...
        console.log(`image counter ${image} + ${image.src}`)
...
window.addEventListener('load', () => {
    const slides = [
        ...
        {href: './images/wakeboard.jpg', title: 'He loves to wakeboard behind his boat'},
    ...
    //prompt for new speed and set new speed
    //get existing spped. display it and accept new speed
    //if slideshow is running pause it, set new speed and resume with new speed else its its already paused, set new speed and let user resume
    $('setSpeed').addEventListener('click',()=>{
        currentSpeed = slideshow.getSpeed()
        let newSpeed = prompt(`Current speed for slides is ${currentSpeed}ms. Enter new speed in milliseconds: `)
        //pause slideshow if currently playing, to reset speed
        if($('play_pause').value === 'Pause'){
            slideshow.createToggleHandler()
            slideshow.setSpeed(newSpeed)
            console.log('createtooglet')
            //play slideshow with new speed
            slideshow.createToggleHandler()
        }else{
            slideshow.setSpeed(newSpeed)
            console.log('createtooglet')
